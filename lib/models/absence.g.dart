// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'absence.dart';

// **************************************************************************
// TypeAdapterGenerator
// **************************************************************************

class AbsenceAdapter extends TypeAdapter<Absence> {
  @override
  final int typeId = 1;

  @override
  Absence read(BinaryReader reader) {
    final numOfFields = reader.readByte();
    final fields = <int, dynamic>{
      for (int i = 0; i < numOfFields; i++) reader.readByte(): reader.read(),
    };
    return Absence(
      id: fields[0] as String,
      employeeId: fields[1] as String,
      typeString: fields[2] as String,
      startDate: fields[3] as String,
      endDate: fields[4] as String,
      note: fields[5] as String?,
      deletedAt: fields[6] as String?,
      version: fields[7] as int,
      updatedAt: fields[8] as String,
      days: fields[9] as int?,
      returnDate: fields[10] as String?,
    );
  }

  @override
  void write(BinaryWriter writer, Absence obj) {
    writer
      ..writeByte(11)
      ..writeByte(0)
      ..write(obj.id)
      ..writeByte(1)
      ..write(obj.employeeId)
      ..writeByte(2)
      ..write(obj.typeString)
      ..writeByte(3)
      ..write(obj.startDate)
      ..writeByte(4)
      ..write(obj.endDate)
      ..writeByte(5)
      ..write(obj.note)
      ..writeByte(6)
      ..write(obj.deletedAt)
      ..writeByte(7)
      ..write(obj.version)
      ..writeByte(8)
      ..write(obj.updatedAt)
      ..writeByte(9)
      ..write(obj.days)
      ..writeByte(10)
      ..write(obj.returnDate);
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is AbsenceAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}

class AbsenceTypeAdapter extends TypeAdapter<AbsenceType> {
  @override
  final int typeId = 0;

  @override
  AbsenceType read(BinaryReader reader) {
    switch (reader.readByte()) {
      case 0:
        return AbsenceType.congeAnnuel;
      case 1:
        return AbsenceType.congeMaternite;
      case 2:
        return AbsenceType.maladie;
      case 3:
        return AbsenceType.sanssolde;
      case 4:
        return AbsenceType.autre;
      default:
        return AbsenceType.congeAnnuel;
    }
  }

  @override
  void write(BinaryWriter writer, AbsenceType obj) {
    switch (obj) {
      case AbsenceType.congeAnnuel:
        writer.writeByte(0);
        break;
      case AbsenceType.congeMaternite:
        writer.writeByte(1);
        break;
      case AbsenceType.maladie:
        writer.writeByte(2);
        break;
      case AbsenceType.sanssolde:
        writer.writeByte(3);
        break;
      case AbsenceType.autre:
        writer.writeByte(4);
        break;
    }
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is AbsenceTypeAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}
